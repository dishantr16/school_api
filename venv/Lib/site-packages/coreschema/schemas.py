...
class Schema(object):
    ...
    def __or__(self, other):
        self_children = self.children if isinstance(self, Union) else [self]
        other_children = other.children if isinstance(other, Union) else [other]
        return Union(self_children + other_children)

    def __and__(self, other):
        self_children = self.children if isinstance(self, Intersection) else [self]
        other_children = other.children if isinstance(other, Intersection) else [other]
...
class Array(Schema):
    ...
    def __init__(self, items=None, max_items=None, min_items=None, unique_items=False, additional_items=True, **kwargs):
        ...
        if (
            ...
            and additional_items is False
            and (max_items is None or max_items > len(items))
        ):
            max_items = len(items)
    ...
    def validate(self, value, context=None):
        ...
        if self.items is not None:
            ...
            for idx, item in enumerate(value):
                if is_list:
                    ...
                    elif isinstance(self.additional_items, bool):
                        break
                    else:
        ...
        if self.unique_items and not (uniq(value)):
...
class String(Schema):
    ...
    def validate(self, value, context=None):
        ...
        if self.pattern is not None and not re.search(self.pattern_regex, value):
            errors += [self.make_error('pattern')]
        if self.format is not None and not validate_format(value, self.format):
...
class Boolean(Schema):
    ...
    def validate(self, value, context=None):
        return [self.make_error('type')] if not isinstance(value, bool) else []
...
class Null(Schema):
    ...
    def validate(self, value, context=None):
        return [self.make_error('type')] if value is not None else []
...
class ExclusiveUnion(Schema):
    ...
    def validate(self, value, context=None):
        matches = sum(child.validate(value, context) == [] for child in self.children)
        if not matches:
            return [self.make_error('match')]
        elif matches > 1:
