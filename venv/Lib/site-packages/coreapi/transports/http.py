...
def _get_method(action):
    return 'GET' if not action else action.upper()


def _get_encoding(encoding):
    return 'application/json' if not encoding else encoding
...
def _get_url(url, path_params):
    ...
    expand the URL.
    """
    return uritemplate.expand(url, path_params) if path_params else url
...
def _get_upload_headers(file_obj):
    ...
    content_disposition = 'attachment; filename="%s"' % name if name else None
    return {
...
def _handle_inplace_replacements(document, link, link_ancestors):
    ...
    * Return the new document as-is.
    ...
    elif link.action.lower() in ('put', 'patch', 'delete'):
        transform = 'inplace'
    else:
        transform = 'new'
