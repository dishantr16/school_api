...
def _get_schema(item, key):
    if schema_data := _get_dict(item, key):
...
def _graceful_relative_url(base_url, url):
    ...
    * Otherwise return the full URL.
    ...
    if url == base_url:
        return ''
    base_prefix = '%s://%s' % urlparse.urlparse(base_url or '')[:2]
    url_prefix = '%s://%s' % urlparse.urlparse(url or '')[:2]
    if base_prefix == url_prefix != '://':
...
def _document_to_primitive(node, base_url=None):
    ...
    if isinstance(node, Document):
        ...
        ret = OrderedDict()
        ret['_type'] = 'link'
        if url := _graceful_relative_url(base_url, node.url):
            ret['url'] = url
        if node.action:
...
class CoreJSONCodec(BaseCodec):
    ...
    def decode(self, bytestring, **options):
        ...
        elif not isinstance(doc, (Document, Error)):
